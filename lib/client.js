...
function Client() {
  ...
  var forms = {
    submit: function(portal_id, form_guid, options, cb) {
      var call = {
        method: 'POST',
  ...
  var events = {
    track: function(options, cb) {
      var call = {
        method: 'GET',
        url: 'https://track.hubspot.com/v1/event/',
        qs: options
      };
      sendUnAuthRequest(call, cb);
    }
  };
  ...
  function sendUnAuthRequest (call, cb) {
    console.log(call);
    request(call, handleResponse(cb));
  }
  ...
  function handleResponse (cb) {
    return function (err, res, data) {
      console.log('handleResponse', err, typeof data === 'string', data);
      if (err) { return cb(err); }
      if (typeof data === 'string' && res.headers['content-type'] !== 'image/gif') {
        try {
