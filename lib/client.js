...
function Client(opts) {
  ...
  var contacts = {
    ...
    search: function(query, options, cb) {
      ...
      sendRequest({
        method: 'GET',
        url: self.baseUrl + '/contacts/v1/search/query',
        qs: options
      }, cb);
    },
  ...
  function sendRequest (call, cb) {
    ...
    limiter.submit(request, call, handleResponse(cb));
  }

  function requestStats() {
    return {
      running: limiter.nbRunning(),
      queued: limiter.nbQueued()
    };
  }
